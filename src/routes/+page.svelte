...
<!-- Hero Section -->
<section class="bg-md-dark-surface-variant py-16">
  <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
    <h1 class="font-display text-4xl md:text-5xl font-bold mb-4">The Solutions Edge</h1>
    <p class="text-xl text-md-dark-on-surface-variant max-w-3xl">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {#each data.posts as post}
          <article class="md-card group">
            <a href="/{post.slug}" class="block">
              <h3 class="font-display text-xl font-semibold mb-2 group-hover:text-md-dark-primary transition-colors">
                {post.title}
              </h3>
    ...
    <aside class="lg:col-span-1">
        ...
        <div class="space-y-2">
          {#each data.categories as category}
            <a 
              ...
              class="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-md-dark-surface-container-high transition-colors"
            >
              <span class="text-md-dark-on-surface-variant">{category.name}</span>
              <span class="text-sm text-md-dark-outline">{category.count}</span>
            </a>
          {/each}
        </div>
      </div>
      ...
      <div class="md-card">
        ...
        <p class="text-md-dark-on-surface-variant text-sm">
